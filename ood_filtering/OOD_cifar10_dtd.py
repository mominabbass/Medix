...
for x in x_axis:
    ...
    # Load dtd as the OOD dataset
    transform_ood = transforms.Compose([transforms.Resize(32), transforms.CenterCrop(32),
                                                           transforms.ToTensor(), transforms.Normalize(mean, std)])
    svhn_train = torchvision.datasets.ImageFolder(root='./data/textures/dtd/images', transform=transform_ood)
    ...
    for inputs, _ in wild_loader:
        ...
        for param in penultimate_layer.parameters():
            if param.grad is not None:
                layer_gradients.append(param.grad.view(-1))
                # print("\nlayer_gradients: ", layer_gradients[-1].size())
